...
class Model(nn.Module):
    def __init__(self, rel_num):
        ...
        # 三组GlobalPointer
        self.globalpointer_ent = GlobalPointer(2)   # batch_size, 2, max_len, max_len
        self.globalpointer_head = GlobalPointer(self.rel_num)  # batch_size, rel_num, max_len, max_len
        self.globalpointer_tail = GlobalPointer(self.rel_num)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        entity_output = self.globalpointer_ent(last_hidden_state, attention_mask_matrix_padding)
        head_output = self.globalpointer_head(last_hidden_state, attention_mask_matrix_padding)
        tail_output = self.globalpointer_tail(last_hidden_state, attention_mask_matrix_padding)
        # print(entity_output.size())   # batch_size, 2, max_len, max_len
        # print(head_output.size())   # batch_size, 44, max_len, max_len
        # print(tail_output.size())    # batch_size, 44, max_len, max_len
